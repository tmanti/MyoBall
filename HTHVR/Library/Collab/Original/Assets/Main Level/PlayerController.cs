...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (thalmicMyo.pose == Pose.WaveOut && rb.velocity.y == 0 || thalmicMyo.pose == Pose.WaveIn && rb.velocity.y == 0 || Input.GetButton("Jump") && rb.velocity.y == 0)
        {
            //position.z += 1f * speed * Time.deltaTime;
            rb.AddForce(new Vector3(0, 5, 0), ForceMode.Impulse);
            // Change material when wave in, wave out or double tap poses are made.
    ...
    void ReturnToOrigin()
    {
        ...
        position.Set(200*lastLevel, 1, 0);
    }
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            transform.Translate(position.x+200, position.y, -position.z);
            lastLevel += 1;
        }
